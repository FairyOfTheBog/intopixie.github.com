...
import { Home, Users, Package, Sword, Heart, Coins, Save, Scroll } from 'lucide-react';
...
INITIAL_STATE.player.inventory[0] = { item: ITEMS[0], quantity: 5, acquiredAt: Date.now() }; // Quartz
INITIAL_STATE.player.inventory[1] = { item: ITEMS[1], quantity: 2, acquiredAt: Date.now() }; // Salad
INITIAL_STATE.player.inventory[2] = { item: ITEMS[6], quantity: 1, acquiredAt: Date.now() }; // Diamond

import { sounds } from './services/soundManager';

export default function App() {
  const [gameState, setGameState] = useState<GameState>(INITIAL_STATE);
  const [activeTab, setActiveTab] = useState<'home' | 'profile' | 'social' | 'inventory' | 'quests'>('home');
  ...
  useEffect(() => {
    ...
    if (savedState) {
      // Merge saved state with INITIAL_STATE to ensure new properties (like quests) are present
      setGameState({
        ...INITIAL_STATE,
        ...savedState,
        player: {
        ...
        // If quests is missing from savedState, it will use INITIAL_STATE.quests
        quests: savedState.quests || INITIAL_STATE.quests,
        chatHistories: savedState.chatHistories || INITIAL_STATE.chatHistories
      });
    ...
    setGameState(prev => {
      ...
      if (existingSlotIndex !== -1) {
        ...
        if (emptySlotIndex !== -1) {
          ...
          // Inventory full - in a real game we'd show a message here
  ...
  const completeQuest = (questId: string) => {
    setGameState(prev => {
      ...
      // Check if requirements are met (for delivery quests)
      ...
      // Remove items if it was a delivery quest
      if (quest.type === 'delivery' && quest.targetItemId) {
    ...
    setGameState(prev => {
      ...
      return {
        ...
        player: {
          ...
          stats: {
            ...prev.player.stats,
            energy: prev.player.stats.maxEnergy, // Restore energy
          }
        },
  ...
  const finishCharacterCreation = () => {
    if (!gameState.player.name.trim()) return;
    setGameState(prev => ({ ...prev, hasCreatedCharacter: true }));
    sounds.playSuccess();
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-4 bg-black/40 backdrop-blur-md overflow-y-auto">
          ...
          >
            ...
            <CharacterMaker 
              ...
              onFinish={finishCharacterCreation}
            />
          </motion.div>
        </div>
      ) : (
        <>
          {/* Header / Stats Bar - Only visible on Home tab */}
          {activeTab === 'home' && (
            <header className="sticky top-0 z-40 flex items-start justify-between px-4 py-3 bg-black/30 backdrop-blur-md border-b-2 border-white/10">
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-4">
                  {/* Profile Icon Button */}
                  <button 
                    onClick={() => handleTabChange('profile')}
                    className="w-12 h-12 pixel-border-inset overflow-hidden bg-white transition-transform active:scale-95 relative"
                  >
                    <img 
                ...
                {/* Money Display - Moved below as requested */}
                <div className="flex items-center gap-1 text-yellow-300 font-pixel text-xs drop-shadow-md ml-1">
                  <Coins size={14} />
              ...
              <div className="flex items-center gap-4">
                <div className="flex flex-col gap-2">
                  <button 
                    ...
                    className="pixel-button-secondary w-8 h-8 flex items-center justify-center p-0 font-pixel text-[12px]" 
                    title="Credits"
                  >
                    ?
                  </button>
          ...
          <main className="flex-1 relative overflow-hidden">
            <div className="absolute inset-0 overflow-y-auto p-4 pb-24 custom-scrollbar">
              <AnimatePresence mode="wait">
                ...
                >
                  {activeTab === 'home' && (
                    <div className="flex flex-col items-center justify-center min-h-full gap-8 py-10 relative">
                        ...
                        {[...(gameState.quests || [])]
                          ...
                          .sort((a, b) => {
                            if (questSortBy === 'dueDate') {
                              const aDue = a.expiresAt || Infinity;
                              const bDue = b.expiresAt || Infinity;
                              return aDue - bDue;
          ...
          <nav className="fixed bottom-0 left-0 right-0 z-40 bg-[#ffffff] border-t-4 border-[#5d4037] px-2 py-4 flex justify-around items-center shadow-[0_-4px_20px_rgba(0,0,0,0.2)]">
            ...
            <NavButton 
              ...
              icon={<img src="https://github.com/FairyOfTheBog/myimgsources/blob/main/mail%20(1).png?raw=true" alt="Social" className="w-10 h-10 pixelated" referrerPolicy="no-referrer" />} 
            />
            <NavButton 
